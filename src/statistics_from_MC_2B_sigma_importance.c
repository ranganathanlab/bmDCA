...
main(int argc, char* argv[])
{
  ...
  for (rip = 0; rip < RIP; rip++) {
    ...
    for (m = 0; m < M; m++) {
      dE[m + M * rip] = 0;
      for (i = 0; i < N; i++) {
        dE[m + M * rip] += h[x[(rip * M + m) * N + i] + i * q] -
                           h_ref[x[(rip * M + m) * N + i] + i * q];
        for (j = i + 1; j < N; j++)
          dE[m + M * rip] +=
            J[x[(rip * M + m) * N + j] + x[(rip * M + m) * N + i] * q +
              q * q * j + i * N * q * q] -
            J_ref[x[(rip * M + m) * N + j] + x[(rip * M + m) * N + i] * q +
                  q * q * j + i * N * q * q];
      }
  ...
  for (i = 0; i < N; i++) {
    ...
    for (a = 0; a < q; a++) {
      ...
      fprintf(
        fpwsigma,
        "%lf ",
        max(sqrt(((double)n1squared[a] - pow((double)n1av[a], 2)) * sqrt(sumw)),
            0));
    }
    fprintf(fpw, "\n");
    fprintf(fpwsigma, "\n");
  ...
  for (i = 0; i < N; i++) {
    ...
    for (j = i + 1; j < N; j++) {
      ...
      for (rip = 0; rip < RIP; rip++)
        for (m = 0; m < M; m++) {
          n2[rip * q * q + (x[(rip * M + m) * N + i]) * q +
             x[(rip * M + m) * N + j]] += p[m + rip * M];
        }
      ...
      for (a = 0; a < q * q; a++) {
        ...
        // if(j==9)printf("%d %d %d %lf %lf %lf %lf %d %d
        ...
        fprintf(fpw2sigma,
                "%lf ",
                max(sqrt(((double)n2squared[a] - pow((double)n2av[a], 2)) *
                         sqrt(sumw)),
                    0));
      }
