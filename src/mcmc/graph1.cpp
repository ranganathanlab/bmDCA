    ...
    for (size_t j = i + 1; j < n; ++j) {
      for (size_t yi = 0; yi < q; ++yi) {
        for (size_t yj = 0; yj < q; ++yj) {
          K[i][j][yi][yj] =
            g2.J[i][j][yi][yj] + 1. / (n - 1) * (g2.h[i][yi] + g2.h[j][yj]);
  ...
  for (size_t i = 0; i < n; ++i) {
    for (size_t j = i + 1; j < n; ++j) {
      for (size_t yi = 0; yi < q; ++yi) {
        for (size_t yj = 0; yj < q; ++yj) {
          K[i][j][yi][yj] =
            g3.L[i][j][yi][yj] + 1. / (n - 1) * (g3.l[i][yi] + g3.l[j][yj]);
          K[j][i][yj][yi] = K[i][j][yi][yj];
        }
      }
    }
...
void
Graph1::randomize_gauss(double beta)
{
  for (size_t i = 0; i < n; ++i) {
    for (size_t j = i + 1; j < n; ++j) {
...
void
Graph1::randomize_gauss2(double beta, double het, double base)
{
  ...
  for (size_t i = 0; i < n; ++i) {
    for (size_t j = i + 1; j < n; ++j) {
      ...
      for (size_t yi = 0; yi < q; ++yi) {
        for (size_t yj = 0; yj < q; ++yj) {
          K[i][j][yi][yj] =
            bi[i] * bi[j] * (base * ((qi[i] == yi) + (qi[j] == yj)) + gasdev());
          K[j][i][yj][yi] = K[i][j][yi][yj];
        }
      }
    }
...
ostream&
Graph1::print_distribution(ostream& os)
{
...
ostream&
Graph1::print_parameters(ostream& os)
{
...
void
Graph1::print_parameters(FILE* of)
{
  log_out << "printing parameters K" << endl;
  for (size_t i = 0; i < n; ++i) {
    for (size_t j = i + 1; j < n; ++j) {
